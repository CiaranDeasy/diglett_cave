...
void HullGUI::draw( 
    ...
    sf::RectangleShape fill = sf::RectangleShape( sf::Vector2f( (float) 
		    ( currentHullFraction * ( GUI_SIZE.x - 2*BORDER_THICKNESS ) ), 
            (float) ( GUI_SIZE.y - 2*BORDER_THICKNESS ) ) );
    fill.setFillColor( FILL_COLOR );
    fill.setPosition( (float) ( position.x + BORDER_THICKNESS ), 
            (float) ( position.y + BORDER_THICKNESS ) );
    target.draw( fill );
    // Draw the text.
    ...
    text.setPosition( (float) ( position.x + (GUI_SIZE.x/2) ), 
            (float) ( position.y + (GUI_SIZE.y/2) ) );
    target.draw( text );
}
